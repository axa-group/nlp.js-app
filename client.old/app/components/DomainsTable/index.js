...
class DomainsTable extends React.Component { // eslint-disable-line react/prefer-stateless-function
  ...
  updateData(props) {
    const initialData = Immutable.asMutable(props.data.domains, { deep: true });
    const data = Immutable.asMutable(props.data.domains, { deep: true });
    const pages = Math.ceil(props.data.total / this.props.defaultPageSize);
    this.setState({
      initialData,
      data,
      pages,
