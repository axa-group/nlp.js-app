...
const Defaults = require('hapi-swagger/lib/defaults');
...
function appendQueryString(url, qsName, qsValue) {
  ...
  if (qsName && qsValue) {
    ...
    urlObj.search = `?${encodeURIComponent(qsName)}=${encodeURIComponent(
      qsValue
    )}`;
  } else {
    urlObj.search = '';
  ...
  plugin.ext('onPostHandler', (request, h) => {
    ...
    if (response.variety === 'view') {
      ...
      if (request.query.tags) {
        settings.jsonPath = appendQueryString(
          settings.jsonPath,
          'tags',
          request.query.tags
        );
      ...
      if (plugin.realm.modifiers.route.prefix) {
        ['jsonPath', 'swaggerUIPath'].forEach(setting => {
          prefixedSettings[setting] =
            plugin.realm.modifiers.route.prefix + prefixedSettings[setting];
        });
      }
...
exports.plugin = {
  ...
  register: (server, options) => {
    ...
    if (settings.cache) {
      ...
      server.method('getSwaggerJSON', getSwaggerJSON, newoptions);
    }
    ...
    server.route([
      {
        ...
        options: {
          ...
          handler: async (request, h) => {
            ...
            if (settings.cache) {
              const { cached, value } = await server.methods.getSwaggerJSON(
                settings,
                request
              );
              const lastModified = cached
                ? new Date(cached.stored)
                : new Date();
              return h
                .response(value)
    ...
    if (settings.documentationPage === true || settings.swaggerUI === true) {
      ...
      server.dependency(['inert', 'vision'], newserver => {
        ...
        if (
          settings.documentationPage === true ||
          settings.swaggerUI === true
        ) {
          server.route([
            {
              method: 'GET',
